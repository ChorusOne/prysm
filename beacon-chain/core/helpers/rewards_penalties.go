...
func IncreaseBalance(state state.BeaconState, idx types.ValidatorIndex, delta uint64, reason balanceupdate.Reason) error {
	...
	state.LogBalanceIncrease(idx, delta, reason)
...
func DecreaseBalance(state state.BeaconState, idx types.ValidatorIndex, delta uint64, reason balanceupdate.Reason) error {
	balAtIdx, err := state.BalanceAtIndex(idx)
	if err != nil {
		return err
	}
	state.LogBalanceDecrease(idx, delta, reason)
